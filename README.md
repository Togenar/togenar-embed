...
| `setDimensions(on?)` | Show or hide the product's measurement overlay; omit the argument to flip it, and render the reply's `on` on your button. Set `dimensions-button="off"` to hide the viewer's ruler icon when your page carries its own measure button. Measurements have to be on for the project. |
